...
export const StaffPortal: React.FC<StaffPortalProps> = ({
  ...
  // 2. Staff Scanner Interface (White Theme, No front icons)
  return (
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm space-y-5">
        ...
        {activeTab === 'camera' && (
          <div className="space-y-3 text-center">
            <div className="relative mx-auto max-w-sm overflow-hidden rounded-2xl bg-slate-900 border border-slate-300 min-h-[260px] flex items-center justify-center">
              ...
              {!isScanning && (
                <div className="p-6 text-center space-y-3">
                  ...
                  <button
                    type="button"
                    onClick={startCamera}
                    className="py-2.5 px-5 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl text-xs shadow-xs transition-colors cursor-pointer whitespace-nowrap"
                  >
                    {t.startCamera}
                  </button>
                </div>
              ...
              {isScanning && (
                <div className="absolute top-3 right-3 z-10">
                  <button
                    type="button"
                    onClick={stopCamera}
                    className="py-1 px-2.5 rounded-lg bg-rose-600 text-white text-xs font-bold shadow-xs cursor-pointer whitespace-nowrap"
                  >
                    {t.stopCamera}
                  </button>
                </div>
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white border border-slate-300 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-slate-900 font-mono uppercase focus:outline-none focus:border-teal-600"
              />
              <button
                type="button"
                onClick={() => verifyTicket(manualCode)}
                className="px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl text-xs transition-colors cursor-pointer whitespace-nowrap"
              >
                {t.lookupBookingBtn}
              </button>
            </div>
        ...
        {scannedResult && (
          <div className="pt-3 border-t border-slate-200 animate-fade-in">
            ...
            {scannedResult.status === 'VALID' && scannedResult.booking && (
              <div className="p-5 rounded-2xl bg-teal-50/70 border border-teal-300 space-y-3 text-left">
                ...
                {/* Booking Info Grid */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-white p-3.5 rounded-xl border border-teal-200 text-xs">
                  <div>
                ...
                <button
                  ...
                  className="w-full py-3 bg-teal-600 hover:bg-teal-700 text-white font-black rounded-xl text-xs sm:text-sm transition-colors shadow-xs cursor-pointer whitespace-nowrap"
                >
                  {t.btnConfirmEntry}
                </button>
              </div>
            ...
            {scannedResult.status === 'ALREADY_USED' && (
              <div className="p-5 rounded-2xl bg-amber-50 border border-amber-300 space-y-2 text-left">
            ...
            {scannedResult.status === 'INVALID' && (
              <div className="p-5 rounded-2xl bg-rose-50 border border-rose-300 space-y-1 text-left">
            ...
            {scannedResult.status === 'DATE_MISMATCH' && scannedResult.booking && (
              <div className="p-5 rounded-2xl bg-orange-50 border border-orange-300 space-y-1 text-left">
